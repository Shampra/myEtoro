...
/*
Porfolio liste
Porfolio complet : portfolio-overview
Porfolio historique ou historique complet : ui-table-hat ou plus haut "w-portfolio-table portfolio-history-grid"
Porfolio item : "w-portfolio-table-hat portfolio-table-view"
Favoris : div.table-body.market
item : div.user-head-content-ph
*/
...
if (sheetData) {
  ...
  var jqxhr = $.getJSON(reqJson, function () {
    ...
    .fail(function (result) {
      console.log("ERREUR DE RECUPERATION : "+result);
    })
    .done(function (data) {
...
//--------------------------------------------------------------------------------------------
  ...
  // Ajout du tri
  if ($("div.inner-header-buttons").find(".menuOrder").length) return;
  else {
    ...
    var menuOrder = "<div class=\"filter dropdown-menu menuFilter ng-scope \"><a class=\"icon\"><div class=\"filter spriteAction\"><img src=\"" + iconType + "\" width=\"22px\" /></div></a><div class=\"drop-select-box myBoxFilter\">";
    menuOrder = menuOrder + "<label class=\"order\" ><a class=\"drop-select-box-option OrderByName\">Nom</a></label>";
    menuOrder = menuOrder + "<label class=\"order\" ><a class=\"drop-select-box-option OrderByER\">ER date</a></label>";
    menuOrder = menuOrder + "<label class=\"order\" ><a class=\"drop-select-box-option OrderByGain\">Gain</a></label>";
    menuOrder = menuOrder + "</div></div>";
    $("div.inner-header-buttons").append(menuOrder);
    $("a.OrderByName").click(function (event) {
      $("div.table-row:not(.empty)").sort(function(a, b) {
        a = $("span.user-nickname", a).text();
        b = $("span.user-nickname", b).text();
        return a.localeCompare(b);
        //a - b;
      }).prependTo('div.table-body:first'); //:first
    });
    $("a.OrderByER").click(function (event) {
      $("div.table-row:not(.empty)").sort(function(a, b) {
        a = $("div.dataER", a).text();
        b = $("div.dataER", b).text();
        return a.localeCompare(b);
    ...
    $("a.OrderByGain").click(function (event) {
      $("div.table-row:not(.empty)").sort(function(a, b) {
